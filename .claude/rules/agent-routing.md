...
|---|---|---|
| "fix and deploy" | coder → pr-reviewer → deployer | 3-step handoff chain |
| "fix this P1" | monitor (triage) → coder → deployer | incident response chain |
| "review and merge and deploy" | pr-reviewer → deployer | 2-step chain |
| "build feature X end to end" | coder → pr-reviewer → deployer | full lifecycle chain |
